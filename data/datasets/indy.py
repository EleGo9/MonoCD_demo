...
from .kitti_utils import Calibration, read_label, approx_proj_center, refresh_attributes, show_heatmap, show_image_with_boxes, show_edge_heatmap, get_3d_dis

from config import TYPE_ID_CONVERSION

class INDYDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True, debug=False):
		super(INDYDataset, self).__init__()
		self.root = root
		...
		self.logger.info("Initializing INDY {} set with {} files loaded.".format(self.split, self.num_samples))
	...
	def check_empty_label(self):
		""" Remove samples where the label file is empty. """
		valid_images = []
		valid_labels = []
	...
	def check_out_bbox_label(self):
		...
		for img, label in zip(self.image_files, self.label_files):
			...
			with open(label_path, "r") as file:
				...
				x1, y1, x2, y2 = content[4:8]
				height = float(x2)-float(x1)
				width = float(y2)-float(y1)
				if float(x2)<img_w and float(y2)<img_h:
					# if height>20. and width>20.: #if you want ot define a limit of bbox dimensions
					valid_images.append(img)
					valid_labels.append(label)
	...
	def crop_image(self, image):
		...
		h, w, c = img.shape
		ret_img = np.zeros((self.input_height, self.input_width, c))
		pad_y = (h-self.input_height) // 2
		pad_x = (w- self.input_width) // 2
		ret_img= img[pad_y:pad_y+h, pad_x: pad_x + w]
		pad_size = np.array([pad_x, pad_y])
	...
	def __getitem__(self, idx):
		...
		img3 = show_image_with_boxes(img, cls_ids, target_centers, bboxes.copy(), keypoints, reg_mask, 
									offset_3D, self.down_ratio, pad_size, orientations, vis=False)
		if self.debug:
			plt.figure(figsize=(10, 6))
			plt.imshow(img3)
		...
		fields = [
			...
			("EL", EL),
			("calib", calib),
			# ("ground_plane", ground_plane) if self.use_ground_plane or self.pred_ground_plane else None,
			# ("horizon_heat_map", horizon_heat_map) if self.pred_ground_plane else None,
			# ("horizon_state", horizon_state) if self.pred_ground_plane else None,
			# ("horizon_vis_img", np.array(img)) if self.pred_ground_plane else None,
			("reg_mask", reg_mask),
			...
			("edge_len", input_edge_count) if self.enable_edge_fusion else None,
			("edge_indices", input_edge_indices) if self.enable_edge_fusion else None
		]
		...
		if self.debug:
			...
			for field_name, field_value in fields:
				if field_value is not None and field_name=='reg_mask':  # Only print fields that are not None
